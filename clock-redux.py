...
def drawProgressBarBox(offscreen_canvas_clock,progressBarXOrigin,progressBarYOrigin,progressBarLength,progressBarHeight):
    ...
    # Draw the outline
    graphics.DrawLine(offscreen_canvas_clock, progressBarXOrigin + 1, progressBarYOrigin, progressBarXOrigin + progressBarLength - 1, progressBarYOrigin, white)
    graphics.DrawLine(offscreen_canvas_clock, progressBarXOrigin + progressBarLength, progressBarYOrigin + 1, progressBarXOrigin + progressBarLength, progressBarYOrigin + progressBarHeight, white)
    graphics.DrawLine(offscreen_canvas_clock, progressBarXOrigin, progressBarYOrigin + progressBarHeight, progressBarXOrigin + progressBarLength, progressBarYOrigin + progressBarHeight, white)
    graphics.DrawLine(offscreen_canvas_clock, progressBarXOrigin, progressBarYOrigin + 1, progressBarXOrigin, progressBarYOrigin + progressBarHeight, white)
...
def drawProgressBar(offscreen_canvas_clock,progressBarComputed, progressBarXOrigin, progressBarYOrigin, progressBarHeight, hour, beginTime, endTime):
    ...
    bred = graphics.Color(255, 0, 0)
    green = graphics.Color(0, 128, 0)
    bgreen = graphics.Color(0, 255, 0)
    white = graphics.Color(128, 128, 128)


    # Set the progress bar colour depending on the time and day
    now = datetime.now()
    currentTime = now.strftime("%H:%M:%S")
    currentDay = now.weekday()

    if currentTime > beginTime and currentTime < endTime and currentDay < 5:
        barColour = red
        barCapColour = bred
    else:
        barColour = green
    ...
    # Hightlight the end of the progress bar with a lighter colour
    graphics.DrawLine(offscreen_canvas_clock, progressBarXOrigin + x + 1, progressBarYOrigin + 1, progressBarXOrigin + x + 1, progressBarYOrigin + progressBarHeight - 1, barCapColour)


# Function to pop up notification area
def notificationPopup(offscreen_canvas_clock, hourFirst, hourSecond, minuteFirst, minuteSecond, secondFirst, secondSecond, smallNumberSpriteFlat):
    progressBarXOrigin = 3
    progressBarYOrigin = 26
    progressBarLength = 25
    progressBarHeight = 5

    while progressBarYOrigin != 0:
        ...
        drawProgressBarBox(offscreen_canvas_clock,progressBarXOrigin,progressBarYOrigin,progressBarLength,progressBarHeight)

        time.sleep(0.041)
        progressBarYOrigin -= 1
        progressBarHeight += 1
        offscreen_canvas_clock = matrix.SwapOnVSync(offscreen_canvas_clock)



# Function to pop down notification area
def notificationPopdown(offscreen_canvas_clock, hourFirst, hourSecond, minuteFirst, minuteSecond, secondFirst, secondSecond, smallNumberSpriteFlat):
    progressBarXOrigin = 3
    progressBarYOrigin = 0
    progressBarLength = 25
    progressBarHeight = 25

    while progressBarYOrigin != 26:
...
def main():
    ...
    # Capture the diplayScene (at some point this will be set by MQTT and this can mover into the main loop)
    displayScene = 1
    ...
    while 1:
        ...
        if displayScene == 1:
            # Display Scene 1 - Small digitla with day progress bar
            ...
            # drawProgressBarBox (offscreen_canvas_clock, 3, 26, 25, 5)
            # progressBarComputed = computeProgress (24, hour, 24)
            # drawProgressBar (offscreen_canvas_clock, progressBarComputed, 3, 26, 5, hour, beginTime, endTime)

            offscreen_canvas_clock = matrix.SwapOnVSync(offscreen_canvas_clock)

        if displayScene == 2:
            # Display Scene 2 - Notification Pop Up

            notificationPopup (offscreen_canvas_clock, hourFirst, hourSecond, minuteFirst, minuteSecond, secondFirst, secondSecond, smallNumberSpriteFlat)
            displayScene = 3

        if displayScene == 3:
            # Display Scene 3 - Notification Pop Down

            notificationPopdown (offscreen_canvas_clock, hourFirst, hourSecond, minuteFirst, minuteSecond, secondFirst, secondSecond, smallNumberSpriteFlat)
            displayScene = 1
